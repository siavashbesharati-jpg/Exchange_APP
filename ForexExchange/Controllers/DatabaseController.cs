    ...
    public class DatabaseController : Controller
    {
        ...
        public async Task<IActionResult> RebuildAllFinancialBalances()
        {
            try
            {
                ...
                // Get all non-deleted manual customer balance history records (including historical records for coherent recalculation)
                var allValidManualRecords = await _context.CustomerBalanceHistory
                    .Where(h => !h.IsDeleted && h.TransactionType == CustomerBalanceTransactionType.Manual) // Include ALL manual records for coherent calculation
                    .ToListAsync();
